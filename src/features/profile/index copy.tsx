...
const Profile: FC = () => {
  ...
  const { isLoggedIn = false, user = {} } = store.auth() as any;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // if (response.err) throw new Error(response.result);
    ...
    <div className="container max-w-4xl py-6 space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {user.is_subscribed
              ? `Your subscription is ${new Date(user.expiry_date!) > new Date() ? "active" : "expired"}`
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
                  ...
                  {user.is_subscribed ? (
                    new Date(user.expiry_date!) > new Date() ? (
                      <span className="text-green-600">
                        Active until {new Date(user.expiry_date!).toLocaleDateString()}
                      </span>
                    ) : (
                      <span className="text-red-600">
                        Expired on {new Date(user.expiry_date!).toLocaleDateString()}
                      </span>
                    )
                  ) : (
                    "Not subscribed"
              ...
              <Button
                onClick={() => {
                  if (!user.is_subscribed || new Date(user.expiry_date!) <= new Date()) {
                    // TODO: Implement subscription flow
                  }
                }}
                variant={user.is_subscribed && new Date(user.expiry_date!) > new Date() ? "outline" : "default"}
              >
                {user.is_subscribed
                  ? new Date(user.expiry_date!) > new Date()
                    ? "Manage Subscription"
